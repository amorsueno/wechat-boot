...
import com.wyj.wechat.util.builder.HCB;
...
public class OCR {
	...
	public static String ocrCode(String imgFilePath, int limitCodeLen){
		...
		if(!f.exists()){
  		...
  		String html;
		try {
			html = HttpUtil.upload(HttpConfig.custom().client(client).url(apiUrl).files(new String[]{imgFilePath},"ocrfile",true).map(getParaMap()));
